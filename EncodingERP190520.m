    ...
    figure(session)
      ...
      switch session
            case 1
                % Encoding
                  folderfiles_save = folderfiles_Enc_save;    
                  clusterEnc = load('clusterEnc.mat'); cluster = clusterEnc.cluster;
%                   cluster{1,1}     =  clusterEnc{1,1}(inda,:); WHY do you take only the last channel?
                Ncond = 3; %otherwise you take only CorrAss
                
            case 2
                % Retrieval
                                folderfiles_save = folderfiles_Ret_save;
                                clusterRetr = load('clusterRet.mat');  
